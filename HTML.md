...
- Аудитория мобильного интернета растет. Поэтому адаптируйте сайт под мобильные
...
`type` для тега `<link>` не указывается, так как его значение `text/css`
...
Используете атрибуты `async` \ `defer` для загрузки некритического JavaScript'а.

При подключении скриптов в теге `<script>` атрибут `type` не указывается,
...
- Не ставьте необязательный закрывающий слеш у одиночных тегов
  (`<img>`, `<br>` и другие).
- Перенос длиных строк производите с выравниванием аттрибутам в одну линию.
...
- Не указывайте значение логическому атрибуту, а сам аттрибут указывайте
  последним.
...
- Не оставляйте семантические теги без классов, если прописывайте им свойства 
